...
public class DepartmentController {
    ...
    /**
     * 새로운 부서를 생성하는 API
     * - 요청 본문에 `DepartmentCreateRequest`를 받음
     * - 생성된 부서 정보를 `DepartmentDto`로 반환
     *
    ...
    /**
     * 부서 목록을 조회하는 API (검색, 정렬, 페이징 지원)
     * - 특정 키워드(nameOrDescription)로 검색 가능
     * - 정렬 기준(`sortField`), 정렬 방향(`sortDirection`) 지정 가능
     * - 커서 기반 페이지네이션 지원 (`idAfter`, `cursor`)
     *
    ...
    /**
     * 부서 정보를 업데이트하는 API
     * - 부서 ID를 기반으로 기존 정보를 수정
     * - 요청된 값이 `null`이면 기존 값을 유지
     *
