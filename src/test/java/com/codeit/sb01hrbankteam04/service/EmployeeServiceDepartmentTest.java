...
//          department.getName().toLowerCase() + i + "@부장example.com", // 이메일 설정
  ...
  /// /          "EMP_" + department.getName().substring(0, 3).toUpperCase() + "_" + i + "_" +
  /// System.nanoTime(), // 유니크한 직원 코드 생성
//          department, // 부서 설정
//          "부장", // 직급 설정
...
//  }
  ...
  void 부서별_직원_분포_조회() {
    ...
    for (EmployeeDistributionResponse response : employeeDistribution) {
      ...
      assertThat(Math.floor(response.getPercentage() * 10.0) / 10.0).isEqualTo(
          expectedResponse.getPercentage());
    }
    // 콘솔 출력 (디버깅용)
